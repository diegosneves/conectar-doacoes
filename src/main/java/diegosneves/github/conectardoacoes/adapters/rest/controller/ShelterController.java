...
public interface ShelterController {
    ...
    ResponseEntity<ShelterInformationResponse> receiveDonation(@RequestBody ReceiveDonationRequest request);


    /**
     * Método GET para encontrar um abrigo pelo email do usuário responsável.
     * <p>
     * Este método recebe uma {@link String} que representa o email do usuário responsável
     * e retorna uma resposta que inclui os detalhes do abrigo encontrado.
     * </p>
     * <p>
     * O método retornará um objeto JSON com os seguintes campos:
     * <ul>
     ...
     * do abrigo.
     *
     * @throws ShelterEntityFailuresException se o usuário informado não for do tipo beneficiário ou
     * não estiver cadastrado na base de dados. A mensagem de erro será incluída na exceção.
     */


    @GetMapping(value = "/findShelter/{userResponsibleEmail}")
    @Operation(
            summary = "Encontra um abrigo pelo email do responsável",
            description = "Este endpoint é responsável por encontrar um abrigo pelo email de seu responsável, " +
                    "utilizando os dados recebidos por meio de uma requisição GET, passando o parâmetro na URL.",
            tags = "Abrigos"
    )
    @ApiResponses(value = {
            @ApiResponse(
